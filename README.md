...
### 作业管理
- 完整的作业队列展示
- **双标签页**: 活跃作业 + 历史任务（过去30天）
- 按状态、分区筛选
- 批量操作（取消、挂起、释放、暂停、恢复）
- **作业详情抽屉** - 参考 `jobinfo` 命令展示详细信息：
  - 基本信息、资源分配、时间信息
  - CPU 使用详情（用户态/系统态/利用率）
  - 内存使用详情（预留/实际使用/利用率）
  - 磁盘 I/O（最大写入/读取）
  - 退出码、等待原因、工作目录、提交命令
- **历史任务分页**: 默认50条/页，支持筛选和搜索
- **密码验证保护敏感操作**
...
- `GET /api/nodes` - 节点列表
- `GET /api/jobs/history` - 历史任务列表（过去30天）
  - 参数: `days` (1-90), `page`, `per_page` (10-100), `state`, `user`, `partition`
  - 返回: 作业列表 + 分页信息
- `GET /api/node/<name>` - 节点详情
- `GET /api/jobs` - 作业列表
- `GET /api/job/<id>` - 作业详情（增强版，参考 jobinfo 脚本）
  - 使用 `sacct` + `sstat` + `squeue` 获取完整信息
  - 返回: 基本信息、资源分配、时间信息、CPU/内存/磁盘使用详情、退出码、工作目录、提交命令
...
### v1.6.0 (2026-03-14)
- **优化组织拓扑功能** - 修正 QOS/账户/用户层级关系展示
  - 忽略 root 根账户，只统计实际账户和用户
  - 支持用户关联多个 QOS（逗号分隔）
  - 支持账户关联多个 QOS（逗号分隔）
  - QOS 节点根据关联情况智能显示账户或用户
  - 用户有独立 QOS 时显示 QOS → User 直接连线
- **新增用户详情抽屉** - 账户管理页面的用户列表支持详情查看
  - 以右侧抽屉方式展示用户信息
  - 显示基本信息：用户名、默认账户、管理员级别
  - 显示关联账户列表及 QOS 信息
  - 显示关联 QOS 列表
- **新增 QOS 关联管理** - QOS 管理页面支持添加/移除关联
  - 支持为账户添加 QOS 关联
  - 支持为用户添加 QOS 关联
  - 支持移除账户/用户的 QOS 关联
...
### v1.4.0 (2026-03-20)
- **作业详情抽屉增强** - 参考 `jobinfo` 脚本，大幅增强详情信息：
  - 新增账户、QOS、退出码信息
  - 新增完整时间线（等待时间、预留/实际运行时间）
  - 新增 CPU 使用详情（用户态/系统态/利用率）
  - 新增内存使用详情（预留/实际使用/利用率）
  - 新增磁盘 I/O（最大写入/读取）
  - 新增提交命令显示（如 Slurm 版本支持）
  - 使用 `sacct` + `sstat` + `squeue` 获取完整信息
- **新增历史任务功能** - 查看过去 30 天的所有作业：
  - 双标签页：活跃作业 + 历史任务
  - 表格展示：预留内存、最大内存、磁盘写入/读取
  - 分页展示：默认 50 条/页，支持筛选和搜索
  - 详情抽屉：点击作业ID查看完整信息
